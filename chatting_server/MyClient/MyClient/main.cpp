...
private:
	...
	void handle_write(const boost::system::error_code& error, size_t bytes_transferred)
	{
		...
		if (pData != nullptr)
		{
			PACKET_DATA* pHeader = (PACKET_DATA*)pData;
			PostSend(true, pHeader->LENGTH, pData);
		}
	}
	...
	void handle_receive(const boost::system::error_code& error, size_t bytes_transferred)
	{
		...
		else
		{
			...
			while (nPacketData > 0)
			{
				if (nPacketData < sizeof(PACKET_DATA))
				{
					break;
				}

				PACKET_DATA* pHeader = (PACKET_DATA*)&m_PacketBuffer[nReadData];

				if (pHeader->LENGTH <= nPacketData)
				{
					ProcessPacket(&m_PacketBuffer[nReadData]);

					nPacketData -= pHeader->LENGTH;
					nReadData += pHeader->LENGTH;
				}
				else
				{
					break;
